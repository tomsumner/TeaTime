...
void derivsc(int *neq, double *t, double *y, double *ydot, double *yout, int *ip)
{
    ...
    for (j=0; j<7; j++){
      for (i=0; i<17; i++){  
        ...
        Lsp_H[i][j] = y[ij+(2*17)];
        Lmn_H[i][j] = y[ij+(3*17)];
        Lmp_H[i][j] = y[ij+(4*17)];  /* NEED TO DOUBLE CHECK THIS */
        Nsn_H[i][j] = y[ij+(5*17)];
        Nsp_H[i][j] = y[ij+(6*17)];
        Nmn_H[i][j] = y[ij+(7*17)];
        Nmp_H[i][j] = y[ij+(8*17)];
        Isn_H[i][j] = y[ij+(9*17)];
        Isp_H[i][j] = y[ij+(10*17)];
        Imn_H[i][j] = y[ij+(11*17)];
        Imp_H[i][j] = y[ij+(12*17];
        ij = ij+1;
      }
    }
    
    ij = 340;   
    for (l=0; l<3; l++){                          /* Susceptible, on ART */
      for (j=0; j<7; j++){
        for (i=0; i<17; i++){
          S_A[i][j][l] = y[ij];
          ij = ij+1;
    ...
    for (i=0; i<17; i++) {
      ...
      for(j=0; j<7; j++){
        TB_deaths[i] = TB_deaths[i] + (Nsn_H[i]+Nsp_H[i]+Nmn_H[i]+Nmp_H[i])*muN_age[i]*muN_H + 
                                      (Isn_H[i]+Isp_H[i]+Imn_H[i]+Imp_H[i])*muI_age[i]*muI_H;
      }                            
    }                                  
