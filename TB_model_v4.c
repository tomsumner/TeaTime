...
void derivsc(int *neq, double *t, double *y, double *ydot, double *yout, int *ip)
{
    ...
    age_in[0] = 0.0;
     ...
     /* Set up vector of midpoints of CD4 categories to use to calculate TB risks */
    double mid_CD4[7] = {500,425,300,225,150,75,25};
    ...
    for (j=0; j<7; j++){
      for (i=0; i<17; i++){
        for (l=0; l<3; l++){
          CD4_dist_ART[j] = CD4_dist_ART[j]+S_A[i][j][l];
        }
      }
    }
   ...
   /* Calculate deaths due to HIV when on ART by age group */
    ...
    for (i=0; i<17; i++){
      for (j=0; j<7; j++){
        for (l=0; l<3; l++){
          ART_deaths[i] = ART_deaths[i]+A_mort[l][j][i]*S_A[i][j][l];
        }
      }
    }
    ...
    for (j=0; j<7; j++){      /* CD4 */
      for (i=0; i<17; i++){   /* age */
        ...
        /* NEED TO INCLUDE HIV RRs IN TB EQUATIONS */
