...
# Set up the forcing functions - all from 1970 onwards
...
# Parameters - proportion primary, proportion smear pos and mortality rates depend on age
fit_cost=0.7
g = fit_cost/(1+fit_cost) # superinfections 

parms <- c(age1 = 1/5, age2 = 1/21, beta = 18.0, 
...
system.time(out_pop <- ode(y=xstart, times, func = "derivsc",
            ...
            outnames = c("Total","Total_S","Total_Ls","Total_Lm","Total_L","Total_Ns","Total_Nm",
                         "Total_N","Total_Is","Total_Im","Total_I","Total_DS","Total_MDR","FS","FM","Mort"), method = rkMethod("rk34f")))
...
# Is TB in equilibrium? # Looks like it's okay but is way higher than spectrum
...
# Check population age structure - need to add up age groups across TB states
...
temp_model1 <- as.data.frame(cbind(seq(1970,2100),tot,out[,"Total"]))
